...
var dashboardMethodSeeds = []authorizationMethodSeed{
	{
		Name:        "dashboard.menu",
		Description: "Sol menüde Dashboard menüsünü gösterir.",
	},
	{
		Name:        "dashboard.view",
		Description: "Dashboard özet verilerini görüntüler.",
		Method:      stringPointer("GET"),
		Path:        stringPointer("/api/v1/dashboard"),
	},
}

func SeedDashboard(db *gorm.DB) error {
	return db.Transaction(func(tx *gorm.DB) error {
		module, err := seedDashboardModule(tx)
		if err != nil {
			return err
		}

		for _, methodSeed := range dashboardMethodSeeds {
			method, err := seedDashboardAuthorizationMethod(tx, module.ID, methodSeed)
			if err != nil {
				return err
			}
...
func seedDashboardAuthorizationMethod(tx *gorm.DB, moduleID uint64, seed authorizationMethodSeed) (ModuleMethodModel, error) {
...
func seedDashboardModule(tx *gorm.DB) (ModuleModel, error) {
	var module ModuleModel
	err := tx.Unscoped().Where("name = ?", "dashboard").First(&module).Error
	if err == nil {
	...
	module = ModuleModel{Name: "dashboard"}
